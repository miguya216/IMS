...
import { generateAssetDetailsPDF } from "/src/pages/Super-admin/forms/functions/AssetDetailsPDF.jsx";
...
const Assets = () => {
  ...
  const fetchAssets = () => {
    fetch("/api/fetch_data.php?action=assets")
      .then((res) => res.json())
      .then((data) => {
        setAssets(data);
        setLoading(false);
  ...
  const handleExport = async () => {
    try {
      ...
      const result = await generateAssetDetailsPDF();
      if (result) {
        setPdfPreviewUrl(result.url);
        setSelectedPDFName(result.filename);
        setShowPdfPreview(true);
      } else {
        console.error("Failed to generate Asset Details PDF");
      }
    } catch (err) {
      console.error("Export error:", err);
    } finally {
      setShowLoading(false); // Always close loading popup
    }
  };
  ...
  return (
    <div className="container-fluid">
      <TableControls
        ...
        addButtonTitle="Register Asset"
        exportButtonTitle="Download Asset Sticker"
        importButtonTitle="Import Asset CSV"
        searchInputTitle="Search by asset name or tag"
      />

      <AssetImport ref={assetImportRef} onImportSuccess={fetchAssets} />
        {loading ? (
          <p className="text-center text-muted">⏳ Loading assets...</p>
