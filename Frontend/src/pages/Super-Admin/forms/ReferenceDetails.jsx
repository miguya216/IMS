...
const ReferenceDetails = ({ type, columns, idField, nameField, assetTypeField }) => {
  ...
  useEffect(() => {
    fetch(`/api/Reference-Data/fetch_ref_data.php?action=${type}s`)
      .then((res) => res.json())
      .then((data) => {
        setData(data);
        setCurrentPage(1);
  ...
  useEffect(() => {
    if (type === "brand") {
      fetch("/api/Reference-Data/fetch_ref_data.php?action=asset_types")
        .then((res) => res.json())
  ...
  const handleSaveConfirmed = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setEditValue("");
        setEditAssetType("");
        const updated = await fetch(`/api/Reference-Data/fetch_ref_data.php?action=${type}s`);
  ...
  return (
    <div className="custom-table-wrapper">
      ...
      <Popups
        ...
        confirmYesNoTitle="Confirm Changes"
