...
class Asset {
    ...
    public function insertAsset($data) {
        ...
        $classification_data = $data['asset_classification'];
        $asset_classification_id = null;

        if (is_numeric($classification_data['existing_id'])) {
            // Existing classification
            $asset_classification_id = $classification_data['existing_id'];
        } else {
            // New classification
            $new_classification = trim($classification_data['new_value']);
            if (empty($new_classification)) {
                return "Asset classification is required.";
            }

            // Check if it already exists
            $stmt = $this->pdo->prepare("SELECT asset_classification_ID FROM asset_classification WHERE asset_classification = ?");
            $stmt->execute([$new_classification]);
            $row = $stmt->fetch();
            if ($row) {
                $asset_classification_id = $row['asset_classification_ID'];
            } else {
                // Create new classification
                $stmt = $this->pdo->prepare("INSERT INTO asset_classification (asset_classification) VALUES (?)");
                $stmt->execute([$new_classification]);
                $asset_classification_id = $this->pdo->lastInsertId();
            }
        }
        ...
        $stmt = $this->pdo->prepare("
            INSERT INTO asset (
                brand_ID, asset_type_ID, asset_classification_ID,
                kld_property_tag, property_tag, responsible_user_ID,
                barcode_ID, qr_ID, room_ID, asset_condition_ID,
                a_source_ID, date_acquired, serviceable_year, price_amount
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ");
        $stmt->execute([
            $brand_id, $asset_type_id, $asset_classification_id,
            $kld_property_tag, $serial_num, $responsible_id,
            $barcode_id, $qr_id, $room_id, $asset_condition_id,
        ...
        $stmt = $this->pdo->prepare("
            INSERT INTO property_card_record (
                ...
                officer_user_ID, room_ID, price_amount, remarks
            ) VALUES (?, ?, ?, ?, ?, ?, ?)
        ");

        // Log the initial manual insertion record
        $stmt->execute([
            $property_card_id,
            'Manual Input',                 
            'REF-INIT-1',                
