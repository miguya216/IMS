...
class ImportCSV {
    ...
    public function importFromCSV($csvFilePath) {
        ...
        while (($row = fgetcsv($handle)) !== false) {
            ...
            [
                ...
                $brand,
                $asset_type,
                $asset_classification, // NEW FIELD
                $acquisition_source,
                $date_acquired_raw,
            ...
            try {
                ...
                $date_acquired = !empty($date_acquired_raw)
                    ? date('Y-m-d', strtotime($date_acquired_raw))
                    : date('Y-m-d');
                ...
                // --- LOOKUPS ---
                $asset_type_id = $this->insertAndGetId(
                    "INSERT INTO asset_type (asset_type) VALUES (?)",
                    'asset_type',
                    $asset_type
                );

                $asset_classification_id = $this->insertAndGetId(
                    "INSERT INTO asset_classification (asset_classification) VALUES (?)",
                    'asset_classification',
                    $asset_classification
                );

                $brand_id = $this->insertAndGetId(
                ...
                $a_source_id = $this->insertAndGetId(
                    "INSERT INTO acquisition_source (a_source_name) VALUES (?)",
                    'acquisition_source',
                    $acquisition_source
                );
                ...
                $price_amount = is_numeric($price_amount_raw)
                    ? number_format((float)$price_amount_raw, 2, '.', '')
                    : '0.00';

                // --- Insert Asset ---
                $stmt = $this->pdo->prepare("
                    INSERT INTO asset 
                    (brand_ID, asset_type_ID, asset_classification_ID, a_source_ID, 
                     kld_property_tag, property_tag, barcode_ID, qr_ID, 
                     date_acquired, serviceable_year, price_amount) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ");
                $stmt->execute([
                    $brand_id,
                ...
                // --- Insert into property_card_record (now includes room_ID) ---
                $stmt = $this->pdo->prepare("
                    INSERT INTO property_card_record 
                    (property_card_ID, reference_type, reference_ID, room_ID, price_amount, remarks)
                    VALUES (?, ?, ?, ?, ?, ?)
                ");

                // We assume imported assets don't have a known room yet — set room_ID to NULL or update this if your CSV has room info
                $stmt->execute([
                    $property_card_id,
                    $reference_type,
                    $ref_no,
    ...
    private function insertAndGetId($query, $table, ...$params) {
        ...
        $idColumn = match ($table) {
            ...
            'asset_classification' => 'asset_classification_ID',
