...
try {
    ...
    // Insert property card record including room_ID
    $stmtInsertPCR = $pdo->prepare("
        ...
        VALUES
            (:property_card_ID, 'IIR', :reference_ID, :officer_user_ID, :room_ID, :price_amount, :remarks)
    ");
    ...
    foreach ($assets as $a) {
        ...
        // Fetch the current room_ID of the asset
        $roomQuery = $pdo->prepare("SELECT room_ID FROM asset WHERE asset_ID = :asset_ID LIMIT 1");
        $roomQuery->execute([':asset_ID' => $asset_ID]);
