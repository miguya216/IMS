...
try {
    ...
    // Add room_ID to property_card_record logic
    ...
    foreach ($assets as $tag) {
        // Get asset details including condition and room
        ...
        if ($asset) {
            ...
            //Fetch the current room of the asset
            $roomQuery = $conn->prepare("SELECT room_ID FROM asset WHERE asset_ID = ? LIMIT 1");
            $roomQuery->execute([$assetID]);
            $currentRoomID = $roomQuery->fetchColumn() ?: null;

            // Insert into ptr_asset
            ...
            if ($propertyCardID) {
                // Insert record into property_card_record with room tracking
                $stmtInsertRecord->execute([
                    $propertyCardID,   // property_card_ID
                    $ptrNo,            // reference_ID
                    $toUser,           // officer_user_ID (new accountable person)
                    $currentRoomID,    // include the current room
                    $priceAmount,      // asset valuation
                    $transferTypeName  // remarks (type of transfer)
                ]);
            }
        }
    }
