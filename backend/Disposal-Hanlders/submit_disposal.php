...
try {
    ...
    // Insert property card record with room_ID
    ...
    foreach ($data['assets'] as $asset) {
        ...
        if ($propertyCard) {
            ...
            // Get asset room_ID
            $stmtGetRoom = $pdo->prepare("SELECT room_ID FROM asset WHERE asset_ID = ?");
            $stmtGetRoom->execute([$asset_ID]);
            $currentRoomID = $stmtGetRoom->fetchColumn() ?: null;
