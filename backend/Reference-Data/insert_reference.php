...
try {
    ...
    switch ($selectedType) {
        ...
        case 'asset_classification':
            $check = $pdo->prepare("SELECT 1 FROM asset_classification WHERE asset_classification = ?");
            $check->execute([$newValue]);
            if ($check->fetch()) {
                echo json_encode(["success" => false, "message" => "Asset type already exists."]);
                exit;
            }
            $stmt = $pdo->prepare("INSERT INTO asset_classification (asset_classification) VALUES (?)");
            $stmt->execute([$newValue]);
            $record_ID = $pdo->lastInsertId();
            break;
