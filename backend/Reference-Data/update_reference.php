...
try {
    switch ($selectedType) {
        ...
        case 'asset_classification':
            $check = $pdo->prepare("SELECT 1 FROM asset_classification WHERE asset_classification = ? AND asset_classification_ID != ?");
            $check->execute([$newValue, $referenceID]);
            if ($check->fetch()) {
                echo json_encode(["success" => false, "message" => "Asset classification already exists."]);
                exit;
            }
            $stmt = $pdo->prepare("UPDATE asset_classification SET asset_classification = ? WHERE asset_classification_ID = ?");
            $stmt->execute([$newValue, $referenceID]);
            logActivity($pdo, $account_ID, "UPDATE", $moduleName, $referenceID, "Updated to: $newValue");
            break;
