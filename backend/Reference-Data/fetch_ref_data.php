...
class RefData {
    ...
    public function getAllAssetClassification($status) {
        $stmt = $this->pdo->prepare("SELECT asset_classification_ID, asset_classification, asset_classification_status FROM asset_classification WHERE asset_classification_status = :status");
        $stmt->execute(['status' => $status]);
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
...
switch ($action) {
    ...
    case 'asset_classification':
        echo json_encode($refData->getAllAssetClassification($status));
        break;
    case 'logs':
