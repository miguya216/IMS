...
const RoomAssignation = () => {
  ...
  // popup states
  const [pendingQrTag, setPendingQrTag] = useState(null);
  ...
  const handleQrSuccess = (decodedText) => {
    ...
    // store scanned tag and show confirm popup
    setPendingQrTag(decodedText);
  ...
  const confirmQrMove = () => {
    ...
    fetch("/api/Room-Handlers/qr_update_asset_room.php", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ tag: pendingQrTag, room_ID: selectedRoom }),
    })
      .then((res) => res.json())
      .then((data) => {
        setShowLoading(false);
        setPendingQrTag(null);

        if (data.success) {
          fetchAssetsByUser();
          setResponseTitle("Success");
          setResponseMessage(data.message || "Asset moved successfully.");
        } else {
          setResponseTitle("Action Failed");
          setResponseMessage(data.message || "Failed to move asset.");
        }
        setShowResponse(true);
      })
      .catch(() => {
  ...
  return (
    <>
      ...
      <div className="container-fluid">
        <div className="row g-3 align-items-end">
          ...
          <div className="col-md-3">
            <label className="form-label fw-semibold d-none d-md-block">Search Asset:</label>
      ...
      />
      {/* 🔹 Confirmation for QR scan */}
      <Popups
        showConfirmYesNo={showQrConfirm}
        confirmYesNoTitle="Confirm QR Transfer"
        confirmYesNoBody={`Move asset with tag "${pendingQrTag}" to room ${roomName}?`}
        confirmYesLabel="Yes, Move"
        confirmNoLabel="Cancel"
        onConfirmYes={confirmQrMove}
        onConfirmNo={() => setShowQrConfirm(false)}
      />
    </>
  );
};
